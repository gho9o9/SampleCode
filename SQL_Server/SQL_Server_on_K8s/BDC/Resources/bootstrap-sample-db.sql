...
WHERE name = @config_option and value_in_use = 1)
BEGIN
...
CREATE OR ALTER PROCEDURE #restore_database
...
BEGIN
	...
	SET @restore_cur = CURSOR FAST_FORWARD FOR SELECT LogicalName, REVERSE(LEFT(REVERSE(PhysicalName), CHARINDEX('\', REVERSE(PhysicalName))-1))
	FROM @files;
	OPEN @restore_cur;
	WHILE(1=1)
	BEGIN
